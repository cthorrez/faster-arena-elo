...
def fit_vectorized_elo(
    ...
    n = matchups.shape[0]
    # if not bootstrapping, just go over the results in order once
    ...
    samp_range = np.arange(num_samples)
    for matchup_indices in sample_indices:
        ...
        model_a_ratings = ratings[samp_range, model_a_indices] # [num_samples,]
        model_b_ratings = ratings[samp_range, model_b_indices] # [num_samples,]
        ...
        ratings[samp_range, model_a_indices] += updates
        ratings[samp_range, model_b_indices] -= updates
    return ratings
